...
class RuleEngineEffectTypesTest {
    private fun getTestRuleEvent(status: EventStatus): RuleEvent {
        return RuleEvent(
            event = "test_event",
    ...
    fun simpleConditionMustResultInAssignEffect() {
        ...
        val ruleEffects = RuleEngine.getInstance().evaluate(getTestRuleEvent(EventStatus.ACTIVE), null, emptyList(), RuleEngineContext(listOf(rule)))
    ...
    fun simpleConditionMustResultInAssignEffectMultipleExecution() {
        val ruleAction = RuleAction("'test_string'", "ASSIGN", mapOf(Pair("field", "test_data_element")))
        val rule = Rule("true", listOf(ruleAction))
        val ruleEffects = RuleEngine.getInstance().evaluateAll(null, listOf(getTestRuleEvent(EventStatus.ACTIVE)), getRuleEngineContext(listOf(rule)))
        assertEquals(1, ruleEffects.size)
    ...
    fun testEnvironmentVariableExpression() {
        ...
        val ruleEffects = RuleEngine.getInstance().evaluate(getTestRuleEvent(EventStatus.COMPLETED), null, emptyList(), RuleEngineContext(listOf(rule)))
        assertEquals(1, ruleEffects.size)
